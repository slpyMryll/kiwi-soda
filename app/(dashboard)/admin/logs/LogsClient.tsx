    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <select 
          ...
          className="px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4332] shadow-sm cursor-pointer"
        >
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-gray-900">{log.profiles?.full_name}</td>
                  <td className="px-6 py-4">
                    <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-[10px] font-black uppercase border border-gray-200">
                      {log.action_type.replace(/_/g, ' ')}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-gray-600 max-w-md truncate">{log.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
