...
export default function Home() {
  ...
  const isEmailValid = useMemo(() => email.toLowerCase().endsWith("@vsu.edu.ph"), [email]);
  const isPasswordValid = useMemo(() => password.length >= 8, [password]);

  const getBorderClass = (isValid: boolean, value: string) => {
    if (!isSubmitted && !value) return "border-gray-300 focus:border-green-dark";
    return isValid 
    ...
    <main className="min-h-screen w-full bg-surface-brand flex flex-col-reverse lg:flex-row items-stretch justify-center lg:px-14 lg:py-1 lg:gap-14">
      <section className="relative flex-1 bg-white rounded-t-[40px] lg:rounded-2xl px-8 py-10 lg:px-14 lg:py-8 shadow-2xl self-end lg:self-center w-full max-w-2xl mx-auto z-10 transition-all duration-500">
        ...
        <div className="text-center lg:text-left mb-8">
            <h1 className="text-2xl lg:text-3xl font-bold text-green-dark mb-2">Welcome to OnTrack</h1>
            <p className="text-gray-500 text-sm">Log in to access USSC updates and budget tracking.</p>
        </div>

        <form action={handleFormAction} className="space-y-6">
          <div className="flex flex-col space-y-2">
            <label className="text-sm font-bold text-green-dark ml-1 text-left">Email</label>
            <div className="relative group">
              <Mail className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${email ? (isEmailValid ? 'text-green-600' : 'text-red-500') : 'text-gray-400'}`} />
              <input
                name="email"
                type="email"
                required
                ...
                className={`w-full bg-white border-b-2 py-4 pl-12 pr-4 text-sm focus:outline-none transition-all ${getBorderClass(isEmailValid, email)}`}
              />
            </div>
            {isSubmitted && !isEmailValid && <p className="text-[10px] text-red-500 ml-1 text-left">Must be a valid @vsu.edu.ph email</p>}
          </div>

          <div className="flex flex-col space-y-2">
            <label className="text-sm font-bold text-green-dark ml-1 text-left">Password</label>
            <div className="relative group">
              <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${password ? (isPasswordValid ? 'text-green-600' : 'text-red-500') : 'text-gray-400'}`} />
              <input
                name="password"
            ...
            {isSubmitted && !isPasswordValid && <p className="text-[10px] text-red-500 ml-1 text-left">Password must be at least 8 characters</p>}
          </div>

          <div className="text-left">
            <button type="button" className="text-xs font-bold text-green-dark hover:underline hover:text-green-800 transition-colors">
              Forgot Password?
            </button>
          </div>

          <button type="submit" className="w-full bg-[#1B4332] text-white font-bold py-4 rounded-2xl shadow-lg transition-all duration-300 lg:hover:bg-green-900 lg:hover:scale-[1.02] active:scale-95">
            Sign In
          </button>

          <div className="relative flex items-center justify-center py-2">
            <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-gray-200"></span></div>
            <span className="relative px-4 bg-white text-gray-400 text-xs">or</span>
          </div>
          ...
          <div className="relative">
            ...
            <div className="p-4 mt-6">
              <p className="text-white text-xl leading-relaxed font-inter opacity-90">
                "Ontrack bridges the gap for students who want to know what the council is doing with their contributions, ensuring every project maintains the utmost security and visibility."
              </p>
            </div>
          </div>
        </div>
