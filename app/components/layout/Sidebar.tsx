...
export function Sidebar({ role }: SidebarProps) {
  ...
  return (
    <aside className="fixed bottom-0 left-0 z-40 w-full h-16 bg-white border-t border-gray-200 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)] flex md:hidden lg:flex lg:shadow-none lg:sticky lg:top-18 lg:w-64 lg:bg-bg-main lg:h-[calc(100vh-72px)] lg:border-t-0 lg:border-r lg:border-gray-200/60 lg:flex-col pb-[env(safe-area-inset-bottom)] lg:pb-0">
      <nav className="flex w-full justify-around items-center px-2 py-3 overflow-x-auto no-scrollbar md:flex-col md:justify-start md:flex-1 md:py-6 md:px-4 md:space-y-1.5 md:items-stretch md:overflow-visible">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "bg-[#E6F4EA] text-green-dark px-4 gap-2 md:bg-surface-accent md:shadow-sm md:px-4 md:w-full"
              ...
              {Icon && (
                <Icon
                  className={cn(
                    ...
                    isActive ? "text-green-dark" : "text-gray-400"
                  )}
                />
              )}
              <span
