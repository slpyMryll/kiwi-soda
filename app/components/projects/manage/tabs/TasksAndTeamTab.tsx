...
export function TasksAndTeamTab({
  ...
  const getMemberStats = (profileId: string) => {
    ...
    return { total: memberTasks.length, completed, percentage: memberTasks.length > 0 ? (completed / memberTasks.length) * 100 : 0 };
  };
    ...
    <div className="flex flex-col gap-6">
                ...
                <form onSubmit={handleAssignTask} className="space-y-4 mt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" name="dueDate" required className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#1B4332]" />
                    </div>
                    <div>
              ...
              {editingTask && (
                <form onSubmit={handleEditTaskSubmit} className="space-y-4 mt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-bold text-gray-700 block mb-1">Due Date</label>
                      <input type="date" name="dueDate" defaultValue={editingTask.rawDueDate ? new Date(editingTask.rawDueDate).toISOString().split('T')[0] : ""} required className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#1B4332]" />
                    </div>
                    <div>
                      <label className="text-sm font-bold text-gray-700 block mb-1">Budget Cost (Optional)</label>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 shrink-0 mt-2 sm:mt-0">
                    ...
                    <span className={`px-3 py-1.5 sm:py-1 rounded-full text-xs font-bold w-fit capitalize text-center ${getStatusStyles(task.status)}`}>
                      {task.status.replace("-", " ")}
                    </span>
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-gray-200 shadow-sm">
                ...
                <form onSubmit={handleAddMember} className="space-y-4 mt-4">
                  <div>
                    ...
                    <select name="profileId" required className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#1B4332]">
                      ...
                      {availablePMs.filter((pm: any) => !localMembers.find((m: any) => m.id === pm.id)).map((pm: any) => (<option key={pm.id} value={pm.id}>{pm.full_name}</option>))}
                    </select>
                  </div>
                  <button type="submit" disabled={isLoading} className="w-full bg-[#1B4332] hover:bg-green-900 text-white font-bold py-3 rounded-xl flex justify-center disabled:opacity-70 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {localMembers.length === 0 ? <p className="text-sm text-gray-500 col-span-1 lg:col-span-2 py-4">No members assigned yet.</p> : (
