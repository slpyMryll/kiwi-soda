...
export function ChartsTab({ project }: { project: any }) {
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="bg-white p-5 sm:p-8 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className={`w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 sm:py-2.5 rounded-xl text-sm font-bold transition-colors ${showSelector ? "bg-gray-100 text-gray-700" : "bg-[#1B4332] hover:bg-green-900 text-white shadow-sm"}`}
        >
          {showSelector ? (
            <X className="w-4 h-4" />
          ) : (
      ...
      {showSelector && (
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-gray-200 shadow-sm animate-in slide-in-from-top-4 fade-in duration-200">
      ...
      {activeCharts.length === 0 ? (
        <div className="bg-white p-8 sm:p-12 rounded-2xl border border-dashed border-gray-300 flex flex-col items-center justify-center text-center">
          <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4">
            <LayoutDashboard className="w-7 h-7 sm:w-8 sm:h-8 text-gray-300" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-gray-900">
            Your dashboard is empty
          </h3>
          <p className="text-xs sm:text-sm text-gray-500 mt-1 max-w-sm px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6">
          {activeCharts.map((chartId) => {
            ...
            return (
              <div
                key={chartId}
                className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-200 shadow-sm relative group animate-in fade-in zoom-in-95 duration-200 flex flex-col"
              >
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-[#E6F4EA] rounded-lg text-[#1B4332] shrink-0">
                      <chartConfig.icon className="w-4 h-4" />
                    </div>
                    <h3 className="text-sm sm:text-base font-bold text-gray-900 pr-4 leading-tight">
                      {chartConfig.title}
                    </h3>
                  </div>
                  ...
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors opacity-100 sm:opacity-0 group-hover:opacity-100 focus:opacity-100 shrink-0"
                    title="Remove Chart"
                  >
                    <X className="w-4 h-4" />
                  </button>
                ...
                <div className="flex-1 w-full min-h-62.5 -ml-2 sm:ml-0">
...
function EmptyChartState({ message }: { message: string }) {
  return (
    <div className="h-62.5 flex items-center justify-center flex-col text-center px-4">
      <BarChart3 className="w-8 h-8 text-gray-200 mb-3" />
      <p className="text-[11px] sm:text-sm font-semibold text-gray-400">
        {message}
      </p>
    </div>
  );
