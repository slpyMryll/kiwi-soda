...
import { ReportDataPoint } from "@/types/admin";
import { Card } from "@/components/ui/card";

export function SystemReportsChart({ data }: { data: ReportDataPoint[] }) {
  return (
    <Card className="p-4 sm:p-6 border-gray-100 shadow-sm rounded-xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-3 sm:gap-0">
        <h3 className="text-sm sm:text-base font-bold text-gray-900">System Reports</h3>
        <div className="flex items-center gap-2">
          <div className="w-6 sm:w-8 h-1.5 rounded-full bg-[#8B5CF6]"></div>
          <span className="text-[11px] sm:text-xs font-bold text-gray-900">Reports</span>
        </div>
      </div>
